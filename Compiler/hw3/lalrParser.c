...
int main( int argc , char *argv[] )
{
    ...
    // build goto table
    parser_table = malloc( sizeof(PARSER) * state_num );
    for( i = 0 ; i < state_num ; i++ )
    {
        parser_table[i] = malloc( sizeof(PARSER) * ( num_of_term  + num_of_nonterm ));
    ...
    shift_reduce_driver();
//    i = make_id("(");    
//    printf(" %d ",go_to_state( 3 , i ) );
    
    //view_token(follow_set_start);
    
    
    //Config_set *test;
    //test = go_to0( state_start->config_set , state_start->config_set->dot );
    //view_config_set(go_to0( state_start->config_set , state_start->config_set->dot ));
...
void fill_frist_set(void)
{
    ...
    for ( G_ptr = G_start ; G_ptr != NULL ; G_ptr = G_ptr->next )
    {
        ...
        if ( strcmp( G_ptr->rhs_string->string , "f" ) == 0 )
        {
            ...
            insert_set( ptr , "f" ); // add λinto first set
        }
    }
...
Config_set *create_config_set( GRAMMAR *rule  , TOKEN *dot)
{
    ...
    create = malloc(sizeof(Config_set));
    create->rule = rule;
    create->dot = dot;
    if( dot != NULL )
    ...
    create->next = NULL;
    create->lookahead = malloc(sizeof(TOKEN));
...
int search_state( Config_set *search_point )
{
    ...
    //if(search_point->set_next != NULL && strcmp( search_point->set_next->rule->lhs , "params" ) == 0 )
    //    view_config_set(search_point);        
...
void shift_reduce_driver()
{
    ...
    push_stack( state_start->statenum );
    //view_stack();
    ...
    while(TRUE)
    {
        ...
        {
            case ERROR:
                 printf("error: state:%d token:%s(%d)\n",stack_top->index,scan_ptr->string,make_id(scan_ptr->string));
                 return;
            ...
            case SHIFT:
                 push_stack( parser_table[stack_top->index][make_id(scan_ptr->string)-1].go_to );
            ...
            case REDUCE:
                 ...
                 pop_stack( count_rhs_num( reduce_rule ) );
                 push_stack( parser_table[stack_top->index][make_id( idtogram(reduce_rule)->lhs )-1].go_to );
